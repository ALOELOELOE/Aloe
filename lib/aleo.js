...
/**
 * Check if the user has a stored bid for this auction
 * Used by AuctionCard to show bid indicators (Bid Placed, Reveal Required, etc.)
 * @param {string} auctionId - Auction identifier
 * @returns {boolean} True if a bid exists in localStorage
 */
export function hasStoredBid(auctionId) {
  return getStoredBid(auctionId) !== null;
}

/**
 * Mark a stored bid as revealed (sets revealed flag without clearing data)
 * Bid data is kept because it's still needed for claim_refund
 * @param {string} auctionId - Auction identifier
 */
export function markBidRevealed(auctionId) {
  const bid = getStoredBid(auctionId);
  if (!bid) return;
  const key = `${STORAGE_KEYS.BID_PREFIX}${auctionId}`;
  localStorage.setItem(key, JSON.stringify({ ...bid, revealed: true }));
}

/**
 * Check if a stored bid has been revealed
 * @param {string} auctionId - Auction identifier
 * @returns {boolean} True if the bid exists and was revealed
 */
export function isBidRevealed(auctionId) {
  const bid = getStoredBid(auctionId);
  return bid?.revealed === true;
}
...
export async function fetchAuctionOnChain(auctionId) {
  try {
    ...
    // Parse all fields from the Aleo struct string
    const auctioneerMatch = raw.match(/auctioneer:\s*(aleo1[a-z0-9]+)/);
    const itemIdMatch = raw.match(/item_id:\s*(\d+)field/);
    const minBidMatch = raw.match(/min_bid:\s*(\d+)u64/);
    ...
    return {
      auctioneer: auctioneerMatch ? auctioneerMatch[1] : null,
      itemId: itemIdMatch ? itemIdMatch[1] : null,
      minBid: minBidMatch ? parseInt(minBidMatch[1], 10) : null,
...
/**
 * Fetch the bid count for an auction from the on-chain mapping
 * @param {string} auctionId - Auction ID (without "field" suffix)
 * @returns {Promise<number>} Number of bids (0 if none)
 */
export async function fetchBidCount(auctionId) {
  const value = await fetchMapping(PROGRAM_ID, "bid_count", `${auctionId}field`);
  if (!value) return 0;
  // Strip type suffix (e.g., "3u64" -> 3)
  const count = parseInt(value.replace(/u64$/, ""), 10);
